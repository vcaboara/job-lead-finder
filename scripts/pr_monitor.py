...
class PRMonitor:
    ...
    def send_notification(self, message: str, pr_data: dict = None):
        """
        Send notification via Slack webhook (if configured) or console.

        Args:
            message: Notification message text
            pr_data: Optional PR metadata for rich formatting
        """
        ...
        if self.slack_webhook_url:
            try:
                ...
                if pr_data:
                    # Add formatted blocks for better Slack presentation
                    blocks = [
                        {
                            "type": "section",
                            ...
                            "elements": [
                                {
                                    "type": "mrkdwn",
                                    "text": (
                                        f"<{pr_data.get('html_url', '')}|View PR #{pr_data.get('number', '')}> • "
                                        f"{pr_data.get('title', '')}"
                                    ),
                                }
                            ],
                ...
                response.raise_for_status()
                logger.debug(f"Slack notification sent: {response.status_code}")

            except Exception as e:
                logger.warning(f"Failed to send Slack notification: {e}")
                # Don't fail the whole process if Slack is down
